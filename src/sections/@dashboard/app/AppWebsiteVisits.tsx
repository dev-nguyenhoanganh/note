...
// @mui
import { Card, CardHeader, Box, CardProps, Stack, styled, Chip, Paper } from '@mui/material';
// components
...
const StyledLabel = styled(Stack)(({ theme }) => ({
  flexDirection: 'row',
  alignItems: 'center',
...
export default function AppWebsiteVisits({
  ...
  const [pointedIndex, setPointedIndex] = useState(0);
  const [dataLabelPosition, setDataLabelPosition] = useState<DataLabelPosition[]>([
    ...Array(chartData[0].data.length).fill(DEFAULT_POSITION),
  ]);

  const chartOptions = useChart({
    chart: {
      events: {
        click(_: MouseEvent<HTMLElement>, __?: ReactApexChart, opt?: FieldValues) {
          if (opt) {
            setPointedIndex(opt['dataPointIndex']);
          }
        },
      },
    },
    ...
    dataLabels: {
      ...
      formatter(val: string | number | number[]) {
        ...
        if (String(val) >= String(HIGH_LEVEL_BREAK_POINT)) {
          return RiskLevel.HIGH;
        }

        if (String(val) <= String(LOW_LEVEL_BREAK_POINT)) {
      ...
      yaxis: [
        {
          ...
          opacity: 0.3,
          label: {
            text: `Danger range ${HIGH_LEVEL_BREAK_POINT} - ${MAX_VALUE}`,
            position: 'left',
            offsetX: 118,
          },
        },
    ...
    yaxis: {
      ...
      labels: {
        formatter: (value?: number, opt?: ApexOptions) => {
          if (opt) {
            return String(value);
          }
  ...
  useEffect(() => {
    const chartContainer = document.querySelector('#apex-chart-container');
    const labels: NodeListOf<HTMLElement> = document.querySelectorAll('.apexcharts-data-labels > rect');

    if (chartContainer === null) {
      return;
    }

    const { x: containerX, y: containerY } = chartContainer.getBoundingClientRect();
    const nextPosition: DataLabelPosition[] = [];

    labels.forEach((item: HTMLElement, index) => {
    ...
    setDataLabelPosition(nextPosition);
  }, []);
  ...
  return (
    <Card id="apex-chart-container" {...other}>
      ...
      <Box sx={{ p: 3, pb: 1 }} dir="ltr">
        <ReactApexChart type="line" series={chartData} options={chartOptions as ApexOptions} width={'100%'} />
      </Box>

      <StyledLabel>
        {chartData[0].data.map(
          (item, index) =>
            item && (
              <Chip
                color="primary"
                size="small"
                variant={pointedIndex === index ? 'filled' : 'outlined'}
                key={index}
                label={String(item)}
              />
            ),
      ...
      {chartData[0].data.map((value, index) => {
        if (value === null) {
          return null;
        }

        let foregroundColor = '--danger';
        let svgIcon = personWalkingSvg;

        if (value > 40) {
          svgIcon = personArmsUpSvg;
          foregroundColor = '--normal';
        }

        if (value > 80) {
          svgIcon = personFallingSvg;
        ...
        return (
          <Paper
            key={index}
            id="apexchart-tooltip"
            className={`tooltip ${foregroundColor}`}
            sx={{
              top: dataLabelPosition[index].top,
              left: dataLabelPosition[index].left,
            }}
          >
            {svgIcon}
          </Paper>
